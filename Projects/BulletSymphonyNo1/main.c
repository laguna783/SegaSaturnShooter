/*
** Bullet Symphony No.1 created by Giancarlos Alvarado.
** This is an ongoing project based on Johannes Fetz's Jo Sega Saturn Engine Shooter Demo.
...
void                my_draw(void)
{
    ...
    jo_printf(1, 1, "Stage: %d  ", level);
    jo_printf(28, 1, "Score: %d  ", ship.score);
#ifdef JO_DEBUG
    jo_printf(1, 2, "%s", jo_get_last_error());
#endif
    if (!gameover)
    {
        draw_ship();
    ...
    else
    {
        ...
        jo_printf(7, 16, "Press Start To Try Again");
    }
...
void			jo_main(void)
{
    ...
    jo_set_background_sprite(&SpriteNewmap1, 0, 0);
    init_game();
